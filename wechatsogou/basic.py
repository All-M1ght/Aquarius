...
class WechatSogouBasic(WechatSogouBase):
    ...
    def _get(self, url, rtype='get', **kwargs):
        ...
        if r.status_code == requests.codes.ok:
            ...
            print('requests status_code error %d' % (r.status_code))
    ...
    def _jiefeng(self):
        ...
        while(max_count < 10) :
            ...
            if hasattr(self, '_ocr'):
                ...
                print(result)
                if 'v_code' not in result :
                    print(u"识别失败，1秒后更换验证码再次尝试，尝试次数：%d" %(max_count))
                    time.sleep(1)
                    continue #验证码识别错误，再次执行
                else:
                    print(u"验证码识别成功 验证码：%s" %(result['v_code']))

                    img_code = result['v_code']
    ...
    def _jianjiaojiefeng(self):
        ...
        bodys['v_pic'] = '''v_pic'''
        bodys['v_type'] = '''v_type'''
        post_data = urllib3.urlencode(bodys)
        ...
        if hasattr(self, '_ocr'):
            ...
            while(max_count < 10):
                ...
                if 'v_code' not in result :
                    ...
                    print(u"若快识别成功 验证码：%s" %(result['v_code']))
